...
(function () {
  ...
  class ZPixelBrush extends DecorationUtils.createDecoratorClass(fabric.PencilBrush, {
    ...
    _addZPixel: function(dict_zPixel) {
      ...
      if (this.arr_zPixel.length > 1 && this.isSameZPixel(dict_zPixel, this.arr_zPixel[this.arr_zPixel.length - 1])) {
        outBool = false;
      } else {
        this.arr_zPixel.push(dict_zPixel);
        outBool = true;
      }
      return outBool;
    },
    ...
    createFZPixelPath: function(arr_zPixel) {
      return arr_zPixel;
    },
    ...
    _finalizeAndAddZPixelPath: function () {
      ...
      // var zPixelPathData = this.arr_zPixel;
      // 
      // var zPixelPath = this.createZPixelPath(zPixelPathData);
      // this.canvas.clearContext(ctx);
      // this.canvas.fire('before:zPixelPath:created', { zPixelPath: zPixelPath });
      // this.canvas.add(zPixelPath);
      // this.canvas.requestRenderAll();
      // zPixelPath.setCoords();
      // this._resetShadow();
      //
      // // fire event 'zPixelPath' created
      // this.canvas.fire('zPixelPath:created', { zPixelPath: zPixelPath });

      var fObj_zPixel = this.createFZPixel(this.arr_zPixel[0]);
      this.canvas.clearContext(ctx);
    ...
    onMouseMove: function (pointer, options) {
      ...
      function onMouseMove__old(brush, pointer, options) { 
        ...
        //   brush._render();
        // }
        if (brush._captureDrawingZPixelPath(pointer) && brush.arr_zPixel.length > 1) {
          // clear bottom canvas
          brush.canvas.clearContext(brush.canvas.contextContainer); 
          brush._render();
        }
    ...
    _render: function (ctx_ignored) {
      ...
      brush.zIndexWrapAround(ctx, {
        ...
        func: function(ctx, brush) {
          ...
          logger.log("logZPixelBrush", "_render", "brush.arr_zPixel:", brush.arr_zPixel);
          
          brush.arr_zPixel.forEach((dict_zPixel) => {
