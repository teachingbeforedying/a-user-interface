...
(function () {
  ...
  class ZPixelBrush extends DecorationUtils.createDecoratorClass(fabric.PencilBrush, {
    ...
    createFZPixelPath: function(arr_dict_zPixel) {
      ...
      const arr_fZPixel = arr_dict_zPixel.map((dict_zPixel) => {
        return this.createFZPixel(dict_zPixel);
      });
      ...
      logger.log("logZPixel", "createFZPixelPath", "screenRect__enveloppe:", screenRect__enveloppe);
      ...
      logger.log("logZPixel", "createFZPixelPath", "zPixelIRect__enveloppe:", zPixelIRect__enveloppe);

      const dict_zPixel = arr_dict_zPixel[0];

      const fGroup__zPixelPath = new fabric.Group(arr_fZPixel, {
        // x:      dict_zPixel.screenRect.x,
        // y:      dict_zPixel.screenRect.y,
      ...
      Utils.zip(arr_dict_zPixel, arr_fZPixel).forEach(([dict_zPixel, fZPixel]) => {
        fZPixel.set({
          // x: fZPixel.x - screenRect__enveloppe.x,
          // y: fZPixel.y - screenRect__enveloppe.y,
          left: dict_zPixel.zPixelCoords.ix - zPixelIRect__enveloppe.x - (fGroup__zPixelPath.width  / 2.0),
          top:  dict_zPixel.zPixelCoords.iy - zPixelIRect__enveloppe.y - (fGroup__zPixelPath.height / 2.0),
        });
        fZPixel.setCoords();
      });


      logger.log("logZPixel", "createFZPixelPath", "fGroup__zPixelPath:", Object.assign({}, fGroup__zPixelPath));
      const fZPixelPath = fGroup__zPixelPath;
