function installFabricUpgrade_fabricXrenderZones(fabric) {
  ...
  fabric.Canvas.prototype.getArr_fObj__candidateSelectable = function () {
    ...
    const dZLevel_number__deep = 5;
    const dZLevel_number__high = 0;
    ...
    if(fabricIntegration) {   //SHU: /!\ global var 
      ...
      const arr_dict_zPixel__existing = Object.values(infiniteCanvas.dict_dict_zPixel__existing__nonPooled);
      ...
      const dict_icObj__candidateSelectable = arr_dict_zPixel__filtered.reduce((acc, dict_zPixel) => {
        ...
        const dict_renderZone   = infiniteCanvas.dict_dict_renderZone__nonPooled[key__zPixelCoords]; 
        
        if(dict_renderZone) {
          const dict_icObj = dict_renderZone.dict_icObj;
